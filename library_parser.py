...
if os.path.exists(FILTER_FILE):
    try:
        ...
        print(f"[Info] Loaded external filters from {FILTER_FILE}")
    except: pass
...
if not SKIP_TRANSLATION and os.path.exists(CACHE_FILE):
    try:
        ...
        print(f"[Info] Loaded {len(translation_cache)} terms from translation cache.")
    except:
        translation_cache = {}
...
HTML_PART_2 = """<li id="filterNotice"></li></ul></div>
    ...
    <script>
        ...
        function openDetails(id, skipHistory = false) {
            ...
            // Clear old images immediately to prevent ghosting
        ...
        function updateCarousel() {
            ...
            const imgUrl = currentImages[currentCarouselIndex];
            const modalImg = document.getElementById("modalImg");
            const modalBlurBg = document.getElementById("modalBlurBg");
            
            // Set the new source
            modalImg.src = imgUrl; 
            modalBlurBg.src = imgUrl;
...
def generate_asset_html(asset_id, asset_name, author_name, web_images, booth_url, folder_path, tags, is_adult, wish_count, price_str, limited=False):
    ...
    img_bytes = get_image_folder_size(folder_path)
...
print(f"[Scan] Found {len(asset_data_list)} assets.")
bulk_translate(all_strings_to_translate)

print(f"[Build] Generating HTML items...")
asset_items_final = []
