...
<head>
    ...
    <style>
        ...
        .file-list { max-height: 200px; overflow-y: auto; padding-right: 5px; margin-bottom: 20px; }
        .file-list::-webkit-scrollbar { width: 4px; }
        .file-list::-webkit-scrollbar-thumb { background: #333; border-radius: 10px; }
        .desc-container { margin-top: auto; border: 1px solid #333; border-radius: 8px; background: #111; overflow: hidden; }
        .desc-content { max-height: 0; transition: max-height 0.4s ease, padding 0.4s ease; overflow-y: auto; font-size: 0.8rem; color: #aaa; line-height: 1.5; white-space: pre-wrap; padding: 0 15px; }
        .desc-content.open { max-height: 350px; padding: 15px; border-top: 1px solid #333; }
        .desc-toggle-btn { width: 100%; background: #1a1a1f; border: none; color: var(--primary); padding: 12px; cursor: pointer; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .desc-toggle-btn:hover { background: #222; }
        .desc-toggle-btn::after { content: '▼'; font-size: 0.6rem; transition: transform 0.3s; }
        .desc-toggle-btn.open::after { transform: rotate(180deg); }
        .desc-content a { color: var(--primary); text-decoration: underline; }
    </style>
</head>
<body>
...
HTML_PART_2 = """<li id="filterNotice"></li></ul></div>
    ...
    <script>
        const translations = {
            ...
            de: { warnDelisted: "<b>⚠️ Nicht mehr gelistet</b> Dieses Asset ist möglicherweise nicht mehr verfügbar.", navTitle: "Booth Bibliothek", optionsBtn: "Optionen ⚙", labelLanguage: "Sprache", labelSort: "Sortierung", optId: "Ordner ID", optNew: "Zuletzt hinzugefügt", optName: "Name (A-Z)", optRel: "Beliebtheit", optSize: "Dateigröße", labelAdult: "Filter", optAll: "Alles zeigen", optHide: "Nicht jugendfrei ausblenden", optOnly: "Nur 18+", labelWidth: "Kartenbreite", labelVisual: "Anzeige", optBlur: "Kein Fokus", optHideIds: "IDs verbergen", optTranslate: "Übersetzte Titel", labelBinary: "Dateien", footBooth: "🛒 Auf Booth ansehen", footFolder: "📂 Ordner öffnen", searchPre: "Suche ", searchSuf: " Artikel...", fileSingular: "Datei", filePlural: "Dateien", moreTags: "+ {n} weitere", hiddenResults: " ({n} durch Filter versteckt)", statItems: "Gesamtanzahl", statSize: "Binärgröße", statImgSize: "Grafikgröße", statSpent: "Voraussichtliche Kosten", statUpdated: "Aktualisiert", labelTopTags: "Häufige Tags", btnDesc: "Beschreibung" },
...
def generate_asset_html(asset_id, asset_name, author_name, web_images, booth_url, folder_path, tags, is_adult, wish_count, price_str, limited=False, description=""):
    ...
    <li class="asset" onclick="openDetails('{asset_id}')" 
        ...
        data-desc-orig="{safe_desc}" data-desc-trans="{safe_desc_trans}">
